#!/bin/bash
# ==========================================
# Delete Shadowsocks Account - FIXED VERSION
# ==========================================
...
delete_shadowsocks_user() {
    ...
    # Delete user from Shadowsocks WS - FIXED: better error handling
    echo -e "${yellow}Removing user from Shadowsocks WS...${nc}"
    if ! jq '(.inbounds[] | select(.tag == "ss-ws").settings.clients) |= map(select(.email != "'"$user"'"))' "$config_file" > "${config_file}.tmp" 2>/dev/null; then
        echo -e "${red}ERROR: Failed to update Shadowsocks WS config (jq error)${nc}" >&2
    ...
    echo -e "${green}✓ User removed from Shadowsocks WS${nc}"
    
    # Delete user from Shadowsocks gRPC if exists
    if jq -e '.inbounds[] | select(.tag == "ss-grpc")' "$config_file" > /dev/null 2>&1; then
        echo -e "${yellow}Removing user from Shadowsocks gRPC...${nc}"
        if jq '(.inbounds[] | select(.tag == "ss-grpc").settings.clients) |= map(select(.email != "'"$user"'"))' "$config_file" > "${config_file}.tmp2" 2>/dev/null; then
            if jq empty "${config_file}.tmp2" 2>/dev/null; then
                mv "${config_file}.tmp2" "$config_file"
                echo -e "${green}✓ User removed from Shadowsocks gRPC${nc}"
            else
    ...
    # Check Shadowsocks WS
    if jq -e '.inbounds[] | select(.tag == "ss-ws") | .settings.clients[]? | select(.email == "'"$user"'")' "$config_file" > /dev/null 2>&1; then
        user_still_exists=true
    fi
    
    # Check Shadowsocks gRPC
    if jq -e '.inbounds[] | select(.tag == "ss-grpc") | .settings.clients[]? | select(.email == "'"$user"'")' "$config_file" > /dev/null 2>&1; then
        user_still_exists=true
    fi
...
cleanup_user_files() {
    ...
    # Client config files
    local client_files=(
        "/home/vps/public_html/ss-$user.txt"
        "/home/vps/public_html/ss-$user.json"
        "/home/vps/public_html/ss-$user.conf"
    )
    ...
    local log_files=(
        "/var/log/create-shadowsocks.log"
...
if delete_shadowsocks_user "$user"; then
