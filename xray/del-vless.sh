...
if [[ "$is_interactive" == "true" ]]; then
    ...
    grep -E "^### " "/usr/local/etc/xray/config.json" | cut -d ' ' -f 2-3 | sort -k2 | while read u_name exp; do
        printf "  %-18s %s\n" "$u_name" "$exp"
    done
...
[[ "$is_interactive" == "true" ]] && echo -e "${yellow}Deleting user from config...${nc}"
if delete_user_from_config "$user" "$exp"; then
    # Restart Xray service
    [[ "$is_interactive" == "true" ]] && echo -e "${yellow}Restarting Xray service...${nc}"
    if systemctl restart xray > /dev/null 2>&1; then
        ...
        # Log the deletion
        echo "$(date): Deleted VLess account $user" >> /var/log/delete-vless.log 2>/dev/null
        
    else
        [[ "$is_interactive" == "true" ]] && echo -e "${red}Error: Failed to restart Xray service${nc}"
        restore_config "$backup_file"
        systemctl restart xray > /dev/null 2>&1
        exit 1
    fi
else
    [[ "$is_interactive" == "true" ]] && echo -e "${red}Error: Failed to delete user from config${nc}"
    restore_config "$backup_file"
    exit 1
fi
