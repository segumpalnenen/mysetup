#!/bin/bash
# ==========================================
# Check VMess Users - FIXED VERSION
# ==========================================
...
if [[ ${#users[@]} -eq 0 ]] || [[ -z "${users[0]}" ]]; then
    ...
    echo -e "${red}=========================================${nc}"
    echo -e "${red}No VMess users found in configuration${nc}"
    ...
    if [[ -f "/usr/local/etc/xray/config.json" ]]; then
        ...
        echo -e "${blue}VMess WS clients count:${nc}"
        jq '[.inbounds[] | select(.tag == "vmess-ws") | .settings.clients[]?] | length' /usr/local/etc/xray/config.json 2>/dev/null || echo "0"
        
        echo -e "${blue}VMess gRPC clients count:${nc}"
        jq '[.inbounds[] | select(.tag == "vmess-grpc") | .settings.clients[]?] | length' /usr/local/etc/xray/config.json 2>/dev/null || echo "0"
    else
...
echo -e "${green}Found ${#users[@]} VMess user(s)${nc}"
echo ""
