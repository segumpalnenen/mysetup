...
get_dom() { cat "/usr/local/etc/xray/domain_$1" 2>/dev/null || echo "Not Set"; }
...
display_header() {
    ...
    echo -e "${white} IP VPS   ${nc}: $MYIP ($country)"
    echo -e "${white} CPU      ${nc}: $cpu_usage"
    echo -e "${white} DATE     ${nc}: $DATE2"
    echo -e "${red}=========================================${nc}"
    echo -e "${blue}             PROTOCOL DOMAINS            ${nc}"
    echo -e "${red}=========================================${nc}"
    echo -e "${white} SSH/VPN  ${nc}: $(get_dom ssh)"
    echo -e "${white} WS/OVPN  ${nc}: $(get_dom ssh_ws) / $(get_dom ovpn)"
    echo -e "${white} VMESS    ${nc}: $(get_dom vmess) (gRPC: $(get_dom vmgr))"
    echo -e "${white} VLESS    ${nc}: $(get_dom vless) (gRPC: $(get_dom vlgr))"
    echo -e "${white} TROJAN   : $(get_dom trojan) (gRPC: $(get_dom trgr))"
    echo -e "${white} SS WS/GR : $(get_dom ssws) / $(get_dom ssgr)"
    echo -e "${white} ZIVPN/NS : $(get_dom zivpn) / $(get_dom slowdns)"
    echo -e "${red}=========================================${nc}"
